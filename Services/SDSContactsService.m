...
- (BOOL)available { return [CNContactStore authorizationStatusForEntityType:CNEntityTypeContacts] == CNAuthorizationStatusAuthorized; }

- (void)loadContactsWithCompletion:(void (^)(NSArray<SDSContactNumber *> *, NSError *))completion {
    NSParameterAssert(completion);
    CNAuthorizationStatus status = [CNContactStore authorizationStatusForEntityType:CNEntityTypeContacts];
    if (status != CNAuthorizationStatusAuthorized) {
        NSError *error = [NSError errorWithDomain:SDSContactsServiceErrorDomain code:1 userInfo:@{NSLocalizedDescriptionKey: @"Contacts access is not already authorized in the host process. SmartDialSIM will not request a new permission prompt."}];
        completion(@[], error);
        return;
    }
